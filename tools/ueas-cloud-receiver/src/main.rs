    ...
    let response = match (req.method(), req.uri().path()) {
        ...
        (&Method::POST, "/verify") => {
            ...
            match verify_request(&body_str) {
                ...
                Err(e) => Response::builder().status(StatusCode::INTERNAL_SERVER_ERROR).body(e).unwrap(),
            }
        }
        _ => Response::builder().status(StatusCode::NOT_FOUND).body("Not Found".into()).unwrap(),
    };
    Ok(response)
}
...
fn verify_request(body: &str) -> Result<String, String> {
    ...
    let build = Command::new("dafny").args(["build", "--target", target, "/tmp/ueas.dfy"]).output().map_err(|e| format!("Build: {}", e))?;
