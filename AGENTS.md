...
- Adding new code in `specification/grammar/`, `implementation/kernel/`, `implementation/backends/`, or `tools/` that
...
- **`tools/`** — user-facing applications (CLI, MCP Server, Playground, DAP, Jupyter).
...
- **No Band-Aid Fixes:** The UI layers (e.g., \pp.js\) and middleware (e.g., Cloudflare Workers) MUST NOT implement logic that parses, patches, formats, or alters the AST, algorithms, or output text generated by the core kernel. The kernel and WASM layer are the single source of truth for algorithmic behavior. If a bug is found in parsing, transpilation, or execution, it MUST be fixed in the core (\implementation/kernel\ or \ackends\).
- **Single Source of Truth for WASM:** All deployments MUST use the compiled binaries generated from `implementation/wasm/`. The WASM kernel is deployed to the Cloudflare Workers MCP Server — the sole execution endpoint. Client applications (playground, IDE integrations) consume the MCP API exclusively; they do NOT bundle or execute WASM locally. Duplication of WASM builds or local execution paths is strictly forbidden.
